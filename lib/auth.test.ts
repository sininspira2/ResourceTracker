import { hasRole, hasAnyRole, getDisplayName, getUserIdentifier, authOptions } from "./auth";
...
describe("auth helpers", () => {
  ...
  describe("getUserIdentifier", () => {
    ...
    it("should return name if discordNickname is not available", () => {
      const newSession = { ...session, user: { ...session.user, discordNickname: null } };
    ...
    it("should return email if discordNickname and name are not available", () => {
      const newSession = { ...session, user: { ...session.user, discordNickname: null, name: null } };
    ...
    it("should return id if discordNickname, name, and email are not available", () => {
      const newSession = { ...session, user: { ...session.user, discordNickname: null, name: null, email: null } };
      expect(getUserIdentifier(newSession as any)).toBe("123");
    });

    it("should return 'unknown' if all are unavailable", () => {
        const newSession = { ...session, user: { ...session.user, discordNickname: null, name: null, email: null, id: null } };
...
describe("authOptions.callbacks.jwt", () => {
    ...
    it("should handle database errors gracefully", async () => {
        (db.onConflictDoUpdate as jest.Mock).mockRejectedValue(new Error("DB Error"));
        global.fetch = jest.fn().mockResolvedValue({
            ok: true,
            json: () => Promise.resolve({ roles: ["role1"], nick: "test-nick" }),
        });
