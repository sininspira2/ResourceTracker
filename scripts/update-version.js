...
async function updateChangelog() {
  try {
    ...
    console.log('2. Minor (1.2.0 → 1.3.0) - New features, non-breaking changes')
    ...
    if (versionChoice === '4') {
      ...
      const versionType = versionChoice === '1' ? 'patch' : versionChoice === '2' ? 'minor' : 'major'
      newVersion = incrementVersion(currentVersion, versionType)
    }
    
    // Get release info
    const title = await question(`Release title (e.g., "Bug Fixes & Improvements"): `)
    const type = await question('Release type (major/minor/patch): ') || 'patch'
    ...
    console.log(`2. Commit your changes: git add -A && git commit -m "Release v${newVersion}"`)
    console.log(`3. Users will see the "What's New" modal on their next visit!`)
