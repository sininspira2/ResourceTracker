...
describe("ImportModal", () => {
  ...
  it("should display an error for invalid file type", () => {
    ...
    const input = screen.getByLabelText(/upload a csv file/i, { selector: "input" });
  ...
  it("should display an error for files exceeding the size limit", () => {
    ...
    const largeFile = new File(["a".repeat(257 * 1024)], "large.csv", { type: "text/csv" });
    const input = screen.getByLabelText(/upload a csv file/i, { selector: "input" });

    fireEvent.change(input, { target: { files: [largeFile] } });

    expect(screen.getByText("File size exceeds the 256KB limit.")).toBeInTheDocument();
  });
  ...
  it("should handle filenames with multiple dots", () => {
    ...
    const validFile = new File(["id,name"], "test.now.csv", { type: "text/csv" });
    const input = screen.getByLabelText(/upload a csv file/i, { selector: "input" });
